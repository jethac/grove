//! Grove desktop host — Tauri commands for the workbench.
//!
//! The engine runs in the webview as WASM (grove-wasm). These commands only
...
mod tests {
    ...
    fn write_export_frame_persists_payload() {
        let dir = std::env::temp_dir().join(format!("grove-ffi-test-{}", std::process::id()));
    ...
    fn read_text_file_reads_utf8() {
        let dir = std::env::temp_dir().join(format!("grove-read-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
